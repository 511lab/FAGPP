function [out] = classification_evaluation(actualLabel,predLabel)
%targets c*N/1*N
%outputs c*N/1*N
[n1,m1]=size(actualLabel);
[n2,m2]=size(predLabel);
if n1>1 && m1>1 && n2>1 && m2>1
    ...
    t = unique(actualLabel);
    o = unique(predLabel);
%     t
%     o
    targets = double(ones(length(t),1)*actualLabel == t'*ones(1,length(predLabel)));
    outputs = double(ones(length(t),1)*predLabel == t'*ones(1,length(predLabel)));
end
